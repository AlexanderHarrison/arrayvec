...
impl<A: Array> ArrayVec<A> {
    ...
    /// Ensure array is not full and index is not oob before calling, otherwise UB
    pub unsafe fn insert_unchecked(&mut self, index: usize, element: A::Item) {
        let len = self.len();
    ...
    /// ensure that an item can be popped before calling, otherwise UB
    pub unsafe fn pop_unchecked(&mut self) -> A::Item {
        let new_len = self.len() - 1;
        self.set_len(new_len);
        ptr::read(self.get_unchecked_ptr(new_len))
    }
    ...
    /// assure index can be removed before calling, otherwise UB
    pub unsafe fn remove_unchecked(&mut self, index: usize) -> A::Item {        
        let len = self.len();

        let start_ptr = self.as_mut_ptr();
        let remove_ptr = start_ptr.add(index);
        let copy_count = len - index - 1;
        let copy_ptr = remove_ptr.add(1);
        
        let ret = remove_ptr.read();
        copy_ptr.copy_to(remove_ptr, copy_count);
        ret
    }
